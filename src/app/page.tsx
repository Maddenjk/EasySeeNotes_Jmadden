...
export default function Home(this: any) {
  ...
  async function saveFile(event:any){
    ...
    if(inputFile.current && text != <></>){
      if(inputFile.current.value != ""){
            await fetch('http://172.31.82.235:5000/save-file',{
              method:"GET"
            }).then(async(response)=>{
  ...
  async function selectNewFile(event:any){
    ...
    await fetch('http://172.31.82.235:5000/save-file-name',{
      method:"POST",
      headers:{
    ...
    await fetch('http://172.31.82.235:5000/convert-pdf-to-text', {
      method:"POST",
      headers:{
  ...
  async function getSavedFileText(filename:string){
    ...
    if(filename != ""){
      ...
      const response = await fetch('http://172.31.82.235:5000/get-saved-text',{
      method:"POST",
      headers:{
        "content-type": "application/json"
      },
...
function FilenamesListComponent() {
  ...
  async function getFileNames(){
    ...
    fetch('http://172.31.82.235:5000/get-filenames')
